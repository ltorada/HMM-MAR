function Gamma = initGamma_random(T,K,D,Pstructure,Pistructure, options)
...
if options.hsmm == 1
    ...
    %%% Chain parameters
    P = ones(K) ./ K;  
    distributed_diag_prob = diag(P) ./ (K-1);      % Make P zero-diagonal for the hsmm FB.
    P = P + distributed_diag_prob;
    P = P - diag(P) .* eye(K);
