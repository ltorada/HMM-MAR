...
for cycle=1:hmm.train.cyc
    ...
    if hmm.train.updateGamma
        ...
        if hmm.K>1 || cycle==1
            % state inference
            [hmm,Gamma,~,Xi] = hsinference(data,T,hmm,residuals,[],XX);
...
% --------------- Added by Luis ------------------------ %
if hmm.train.hsmm == 1  % If training with the HSMM FB version:

    % 1.- Get likelihood, which will be used by the HSMM Viterbi wrapper in hmmmar.m
    [hmm, Gamma,~,Xi] = hsinference(data,T,hmm,residuals,[],XX);
    setxx
    hmm.L = obslike([],hmm,residuals,XX,hmm.cache);
    
    % 2.- Make the transition matrix zero-diagonal.
    %distributed_diag_prob = diag(hmm.P) ./ (hmm.K-1);
    %hmm.P = hmm.P + distributed_diag_prob;
    %hmm.P = hmm.P - diag(hmm.P) .* eye(hmm.K);
...
if hmm.train.verbose
    ...
    if hmm.train.useMEX==0 && hmm.train.hsmm == 0
